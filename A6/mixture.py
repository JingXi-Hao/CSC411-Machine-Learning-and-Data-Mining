...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        denominator = np.sum(R) + (R.shape[1] * self.prior.a_mix) - R.shape[1]
    ...
    def update_theta(self, X, R):
        ...
        ######################## Your code here #########################
        numerator = self.prior.a_pixels - 1. + np.dot(R.T, X)
        denominator = self.prior.a_pixels + self.prior.b_pixels - 2. + np.sum(R, axis=0)
    ...
    def compute_posterior(self, X, M=None):
        ...
        ######################## Your code here #########################
        temp_1 = np.multiply(X, M)
        temp_2 = np.multiply(1. - X, M)
        
        log_p_x_given_z = np.dot(temp_1, np.log(self.params.theta).T) + \
                          np.dot(temp_2, np.log(1. - self.params.theta).T)
        log_p_z_x = log_p_x_given_z + np.log(self.params.pi)
